import JointCircleSection from "@/components/joint-circle/joint-circle-section";
import ScrollServiceCards from "@/components/services/scroll-service-cards";
import ServiceSection from "@/components/services/services-section";
import ServicesVideos from "@/components/services/services-video";
import BorderGrid from "@/components/shared/border-grid";
import { SITE_CONFIG } from "@/constants/site";
import { Metadata } from "next";

export const metadata: Metadata = {
  title: `Services | ${SITE_CONFIG.name}`,
};

const Services = () => {
  return (
    <>
      <section className="relative bg-white py-11 overflow-hidden">
        <BorderGrid />
        <ServiceSection />
      </section>
      <section className="relative h-20 md:h-40 2xl:h-[164px] overflow-hidden">
        <BorderGrid />
        <ServicesVideos />
      </section>
      <section className="relative bg-white py-11 overflow-x-clip">
        <BorderGrid />
        <ScrollServiceCards />
      </section>
      <section className="relative bg-gray-dark py-11 overflow-hidden">
        <BorderGrid />
        <JointCircleSection />
      </section>
    </>
  );
};

export default Services;
